...
class Wbty_audio_managerModelaudio_file extends WbtyModelAdmin
{
	...
	protected function populateState()
	{
		...
		$this->setState('audio_file.id', $pk);
	...
	protected function saveRelational($table, $key, $array) {
		...
		$query = 'DELETE FROM #__wbty_audio_manager_'.$table.' WHERE audio_files_id='.$this->table_id;
		$this->_db->setQuery($query)->query();
		
		if ($array) {
			foreach ($array as $id) {
				$query = 'INSERT INTO #__wbty_audio_manager_'.$table.' SET '.$key.'='.$id.', audio_files_id='.$this->table_id;
				$this->_db->setQuery($query)->query();
			}
		}
	...
	public function getRelational($table, $key, $id) {
		$query = 'SELECT '.$key.' FROM #__wbty_audio_manager_'.$table.' WHERE audio_files_id='.$id;
	...
	protected function prep_link($data, $table, $prefix='#__wbty_audio_manager_') {
		...
		$this->_db->setQuery('SELECT * FROM '.$prefix.$table.' WHERE audio_file_id='.$this->table_id.'');
	...
	protected function link_insert($data, $table, $prefix) {
		...
		foreach ($data as $datum) {
			...
			if ($datum['id']) {
				$q = "UPDATE ".$prefix.$table." SET ".$value." WHERE id = '{$datum['id']}' AND audio_file_id = '{$this->table_id}'";
			} else {
