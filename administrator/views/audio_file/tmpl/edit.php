...
<form action="<?php echo JRoute::_('index.php?option=com_wbty_audio_manager&layout=edit&id='.(int) $this->item->id).'&tmpl=' . JRequest::getVar('tmpl'); ?>" method="post" name="adminForm" id="audio_file-form" class="form-validate form-horizontal" enctype="multipart/form-data">

    <?php if (JRequest::getVar('tmpl')=='component') {echo JToolbar::getInstance('toolbar')->render();} ?>
	<div class="width-50 fltlft" style="clear:both;">
		<fieldset class="adminform">
			...
			<div<?php if (strpos($this->form->getInput('include_in_list'), 'type="hidden')===FALSE) { echo ' class="control-group"'; } ?>>
                <?php echo str_replace('<label', '<label class="control-label"', $this->form->getLabel('include_in_list')); ?>
                <div class="controls">
                	<?php echo $this->form->getInput('include_in_list'); ?>
                </div>
         	</div>
